...
if Chef::Config[:solo]
  ...
  search(:node, node[:gitzone][:search_query_managed_nodes] + ' AND root_ssh_pub_keys:*').each do |n|
    node.set[:gitzone][:managed_nodes_pub_keys] = (node[:gitzone][:managed_nodes_pub_keys] + n[:root_ssh_pub_keys]).uniq.sort # unless n[:root_ssh_pub_keys].nil?
  end
...
if !ssh_keys.nil? && ssh_keys.length > 0
  ...
  if File.exist?(authorized_keys_file) && !node[:gitzone][:rewrite_authorized_keys_file]
...
bash 'gitzone-git-init' do
  ...
  code <<-EOF
     ...
     EOF
  not_if { ::Dir.exist?(::File.join(zone_dir, '.git')) }
end
