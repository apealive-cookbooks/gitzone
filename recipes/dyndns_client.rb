...
if Chef::Config[:solo]
  Chef::Log.error('This recipe uses search. Chef Solo does not support search unless you install the chef-solo-search cookbook.')
end
...
unless gitzone_servers.nil?
  gitzone_servers.next do |n|
    # check pub key infrastructure is configured
    # once managed by gitzone create update-record scripts
    if (!n.nil? && !n[:gitzone].nil? && !n[:gitzone][:managed_nodes_pub_keys].nil?) && (n[:gitzone][:managed_nodes_pub_keys].include?(server_root_pub_key))
